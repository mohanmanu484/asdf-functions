...
exports.helloWorld = functions.https.onRequest((request, response) => {
  ...
  admin.database()
    .ref('/followers/' + (request.query.id))
    .push(request.query.fId)
    .then(snapshot => {
...
function updatePoemListForUser(authorId, poemId, value) {

  // auto triggered when new item added in poem list section
  return admin.database()
    .ref(`/users/${authorId}/poems/${poemId}/`)
    .set(value);
}


exports.addPoem = functions.database.ref('/poems/{poemId}/')
  .onWrite(event => {

    const currentVal = event.data.current;
    const prevVal = event.data.previous;
    const poemId = event.params.poemId;

    if (currentVal.val() && !prevVal.val()) {

      const poem = event.data.val();
      const authorId = poem.authorId;
      console.log("author id is ", authorId);
      console.log("poem is ", poem);
      return updatePoemListForUser(authorId, poemId, true);

    } else if (!currentVal.val() && prevVal.val()) {

      console.log("Poem deleted ", `${poemId}`);
      console.log("previous obj exists  " + event.data.previous.exists());
      const poem = event.data.previous.val();
      const authorId = poem.authorId;
      console.log("previous author is " + poem.authorId);
      //  return console.log("previous author is " + poem.authorId);
      return updatePoemListForUser(authorId, poemId, null);
    } else {
      // value updated
      console.log('Updated');
    }
...
exports.followersCount = functions.database.ref('/followers_v2/{followedId}/{followerId}')
  .onWrite(event => {
    ...
    const followerRef = admin.database()
      .ref(`users/${followedUid}/stats`)
      .child("user_followers_count");
...
exports.sendFollowerNotification = functions.database.ref('/followers_v2/{followedUid}/{followerUid}')
  .onWrite(event => {
    ...
    const getDeviceTokensPromise = admin.database()
      .ref(`/users/${followedUid}/notificationToken`)
      .once('value');
...
exports.followers = functions.database.ref("/followers/{followedUid}/{followerId}")
  .onWrite(event => {
    ...
    return admin.database()
      .ref(`followers/${_followedUid}/${_followerId}`)
      .once('value')
      .then(result => {
...
exports.sendNote = functions.database.ref('/test/{some}/')
  .onWrite(event => {
          ...
          .then(response => {
            ...
            response.results.forEach((result, index) => {
              ...
              if (error) {
                ...
                if (error.code === 'messaging/invalid-registration-token' ||
                  error.code === 'messaging/registration-token-not-registered') {
                  tokensToRemove.push(tokensSnapshot.ref.child(keysOfTokens[index])
                    .remove());
                }
              }
            });
